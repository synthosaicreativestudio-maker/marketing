...
from utils import create_specialist_button, sanitize_ai_text_plain, sanitize_ai_text, _is_ai_asking_for_escalation, _strip_reasoning_tags
...
async def _process_ai_response(update, context, ai_service, appeals_service, text, profile_context="", complexity=None, use_rag=True, cascade_level=2):
    ...
    try:
        ...
        # Защитный слой: ДОПОЛНИТЕЛЬНАЯ гарантия очистки (особенно важно если поток оборвался)
        clean_response = _strip_reasoning_tags(full_response)
        clean_response = clean_response.replace("[ESCALATE_ACTION]", "").strip()
