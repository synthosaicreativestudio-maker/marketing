...
def web_app_data_handler(auth_service: AuthService):
    ...
    async def handle_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # Маршрутизация на аналитику
        if data.get('action') == 'analytics_request':
            if settings.ENABLE_ANALYTICS:
                from handlers.analytics import handle_analytics_webapp_data
                await handle_analytics_webapp_data(update, context)
            else:
                await update.message.reply_text("⚠️ Модуль аналитики временно недоступен.")
            return
