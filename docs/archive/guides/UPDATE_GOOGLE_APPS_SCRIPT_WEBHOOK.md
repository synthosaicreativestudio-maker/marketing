...
const WEBHOOK_URL = 'http://89.169.176.108:8080/webhook/promotions';
...
Откройте в браузере:
```
http://89.169.176.108:8080/webhook/promotions
```
...
3. **Проверьте WEBHOOK_SECRET:**
   - На сервере: `grep WEBHOOK_SECRET /home/marketing/marketingbot/.env`
...
1. Проверьте `WEBHOOK_SECRET` на сервере:
   ```bash
   grep WEBHOOK_SECRET /home/marketing/marketingbot/.env
   ```
