...
1. Пользователь отправляет `/start`.
2. Бот отправляет кнопку с WebApp.
3. Пользователь вводит данные в Mini App.
4. Данные отправляются боту через `sendData`.
5. Бот проверяет данные через Google Sheets.
6. Пользователь получает результат (успех/ошибка).
