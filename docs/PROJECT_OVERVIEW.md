...
*   **Назначение:** Предоставляет интерактивный веб-интерфейс для ввода данных авторизации сотрудниками.
*   **Расположение:** `docs/index.html` (для развертывания через GitHub Pages).
*   **Взаимодействие:** Запускается из Telegram бота через inline-кнопку. Отправляет данные боту для обработки.