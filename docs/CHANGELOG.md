...
## [2.1.0] - 2025-09-17 - Исправление авторизации через Mini App

### ➕ Добавлено
- Новый обработчик для получения и обработки данных из Mini App.
- Логика повторной авторизации в случае ошибки.
- Улучшенное логирование для упрощения отладки.

### 🐛 Исправлено
- Проблема с авторизацией через мобильную версию Telegram.
- Стабильная работа метода `Telegram.WebApp.sendData()` с использованием `KeyboardButton` и `ReplyKeyboardMarkup`.

### 🧪 Тестирование
- Проведено тестирование на мобильных устройствах (iOS и Android).

### 🔄 Изменено
- Поток данных для авторизации через Mini App обновлен для повышения надежности.
