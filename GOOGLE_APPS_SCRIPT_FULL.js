...
/**
 * Устанавливает заголовки на листе "Список сотрудников для авторизации".
 * Эта функция должна быть запущена один раз для создания листа с нужными заголовками.
 */
function setupAuthorizationSheet() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheetName = "Список сотрудников для авторизации";
  var sheet = ss.getSheetByName(sheetName);

  if (!sheet) {
    sheet = ss.insertSheet(sheetName);
    Logger.log("Лист '" + sheetName + "' создан.");
  } else {
    Logger.log("Лист '" + sheetName + "' уже существует.");
  }

  var headers = ["Код партнера", "ФИО партнера", "Телефон партнера", "Статус авторизации", "Telegram ID"];
  ...
  Logger.log("Заголовки успешно добавлены/обновлены на листе '" + sheetName + "'.");
  SpreadsheetApp.getUi().alert("Заголовки на листе '" + sheetName + "' установлены.");
}

/**
 * Синхронизирует список сотрудников с листа-источника на лист авторизации.
 * Оптимизированная версия: bulk operations, динамические заголовки, отсутствие пустых строк.
 */
function syncEmployeeList() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();

  // --- Конфигурация ---
  var sourceSheetName = "1. 3. список сотрудников для авто";
  var destinationSheetName = "Список сотрудников для авторизации";
  var sourceStartRow = 3; // Данные начинаются с 3 строки
  var destinationStartRow = 2; // Данные начинаются со 2 строки
  var keywordForEnd = "Итого";
  ...
  var destSheet = ss.getSheetByName(destinationSheetName);

  if (!sourceSheet || !destSheet) {
    SpreadsheetApp.getUi().alert("Ошибка: Не найден лист '" + sourceSheetName + "' или '" + destinationSheetName + "'.");
    return;
  }

  // --- 1. Поиск индексов столбцов динамически ---
  var sourceHeaders = sourceSheet.getRange(1, 1, 3, sourceSheet.getLastColumn()).getValues(); 
  // Ищем заголовки. Обычно они в 1 или 2 строке. Проверим первые 3.
  function findColIndex(headersMatrix, name) {
    for (var r = 0; r < headersMatrix.length; r++) {
      var idx = headersMatrix[r].indexOf(name);
      if (idx !== -1) return idx;
    }
    return -1;
  }
  ...
  var sIdx = {
    code: findColIndex(sourceHeaders, colNames.code),
    name: findColIndex(sourceHeaders, colNames.name),
    phone: findColIndex(sourceHeaders, colNames.phone)
  };

  var destHeaders = destSheet.getRange(1, 1, 1, destSheet.getLastColumn()).getValues()[0];
  var dIdx = {
    code: destHeaders.indexOf(colNames.code),
    name: destHeaders.indexOf(colNames.name),
    phone: destHeaders.indexOf(colNames.phone),
    status: destHeaders.indexOf(colNames.status),
    tgId: destHeaders.indexOf(colNames.tgId)
  };

  // Проверка критических столбцов
  if (sIdx.code === -1 || dIdx.code === -1 || dIdx.status === -1) {
    SpreadsheetApp.getUi().alert("Ошибка: Не удалось найти необходимые заголовки столбцов.");
    return;
  }

  // --- 2. Чтение данных (Bulk) ---
  var sourceValues = sourceSheet.getRange(sourceStartRow, 1, sourceSheet.getLastRow() - sourceStartRow + 1, sourceSheet.getLastColumn()).getValues();
  var destLastRow = destSheet.getLastRow();
  var destValues = destLastRow >= destinationStartRow ? 
                   destSheet.getRange(destinationStartRow, 1, destLastRow - destinationStartRow + 1, destSheet.getLastColumn()).getValues() : [];

  // --- 3. Подготовка данных ---
  var destMap = {}; // Карта существующих сотрудников для сохранения статуса
  destValues.forEach(function(row) {
    var code = String(row[dIdx.code]).trim();
    if (code) {
      destMap[code] = {
        status: row[dIdx.status] || "не авторизован",
        tgId: row[dIdx.tgId] || ""
      };
    }
  });
  ...
  sourceValues.forEach(function(row) {
    var code = String(row[sIdx.code]).trim();
    
    // Пропускаем "Итого" или пустые коды
    if (!code || code.toLowerCase() === keywordForEnd.toLowerCase()) return;

    var existing = destMap[code];
    var newRow = new Array(destHeaders.length).fill("");
    
    newRow[dIdx.code] = code;
    newRow[dIdx.name] = sIdx.name !== -1 ? row[sIdx.name] : "";
    newRow[dIdx.phone] = sIdx.phone !== -1 ? row[sIdx.phone] : "";
    
    if (existing) {
      newRow[dIdx.status] = existing.status;
      newRow[dIdx.tgId] = existing.tgId;
    } else {
  ...
  // --- 4. Запись данных (Bulk) ---
  // Очищаем старые данные (кроме заголовков)
  if (destLastRow >= destinationStartRow) {
    destSheet.getRange(destinationStartRow, 1, destLastRow - destinationStartRow + 1, destSheet.getLastColumn()).clearContent();
  }

  // Записываем новый чистый список без дырок
  if (finalData.length > 0) {
    destSheet.getRange(destinationStartRow, 1, finalData.length, finalData[0].length).setValues(finalData);
    Logger.log("Синхронизировано сотрудников: " + finalData.length);
  }

  // --- 5. Оформление ---
  applyAuthFormatting(destSheet, dIdx.status, destinationStartRow, finalData.length);
  
  SpreadsheetApp.getUi().alert("Синхронизация завершена. Обработано " + finalData.length + " сотрудников.");
}

/**
 * Вспомогательная функция для форматирования статусов.
 */
function applyAuthFormatting(sheet, statusColIdx, startRow, numRows) {
  if (numRows <= 0) return;
  
  var range = sheet.getRange(startRow, statusColIdx + 1, numRows, 1);
  sheet.clearConditionalFormatRules();
