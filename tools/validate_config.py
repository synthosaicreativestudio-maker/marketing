...
def validate_env_vars() -> list[tuple[str, str]]:
    ...
    if token and token.count(':') != 1:
        issues.append((
            'error',
            'TELEGRAM_BOT_TOKEN format appears invalid '
...
def validate_dependencies() -> list[tuple[str, str]]:
    ...
    try:
        ...
        if not hasattr(telegram, '__version__'):
            ...
            if not version.startswith('21.'):
                issues.append((
                    'warning',
                    f'python-telegram-bot version {version} may not be '
                    'compatible (expected 21.x)'
                ))
    except ImportError:
    ...
    if not importlib.util.find_spec('gspread'):
        issues.append((
            'warning',
            'gspread is not installed (Google Sheets integration will not work)'
        ))
